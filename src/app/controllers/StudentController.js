...
class StudentController {
  async viewFee(req, res) {
    try {
      ...
      //res.json(result);
      res.render("../../resources/user/student/money.hbs", {account:  account,FeeInfor:  result});
    } catch (error) {
  ...
  try {
    ...
    res.render('../../resources/user/student/suathongtin.hbs', { student: studentData }, (err, html) => {
      if (err) {
        console.error("Error rendering template:", err);
        res.status(500).json({ message: "Error rendering template" });
        return;
